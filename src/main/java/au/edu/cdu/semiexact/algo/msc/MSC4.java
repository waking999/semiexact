...
public class MSC4<ET, ST> implements IMSC<ET, ST> {
	 ...
	 * @return
	 */
	protected int buildMaxMatching(MSCGlobalVariable<ET, ST> gv, int[] card, int[] freq) {
	 ...
	 * @return
	 */
	protected boolean preProcess(MSCGlobalVariable<ET, ST> gv, int[] card, int freq[]) {
	...
	protected int kHighest(MSCGlobalVariable<ET, ST> gv, int[] card, int maxCard, int sActCount) {
	...
	private int branch(MSCGlobalVariable<ET, ST> gv, int[] card, int[] freq) {
		int bestSolCount = gv.getBestSolCount();
		int solCount = gv.getSolCount();
		int s1 = card[0];
		int e1 = freq[0];
